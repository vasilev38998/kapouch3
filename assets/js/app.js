      ...
      return `<article class="card live-order" data-live-id="${item.id}">
        ...
        <div class="row" style="justify-content:space-between;align-items:center">
          ...
          <select data-live-status>
            ${[{v:'created',l:'создан'},{v:'accepted',l:'принят'},{v:'paid',l:'оплачен'},{v:'preparing',l:'готовится'},{v:'ready',l:'готов'},{v:'done',l:'выдан'},{v:'cancelled',l:'отменён'}].map((x) => `<option value="${x.v}" ${x.v===item.status?'selected':''}>${x.l}</option>`).join('')}
          </select>
        </div>
      </article>`;
    }).join('');
