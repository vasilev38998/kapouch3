...
class ProfileController {
    public function index(): void {
        ...
        $ps = $pdo->prepare("SELECT id, provider, external_order_id, amount, status, payload_json, created_at FROM payment_sessions WHERE user_id=? AND provider IN ('tinkoff_sbp','internal_balance','internal_balance_rub') AND status IN ('accepted','paid','preparing','ready','done') ORDER BY created_at DESC LIMIT 30");
        $ps->execute([$user['id']]);
        $payStatusMap = ['accepted' => 'оплачен', 'paid' => 'оплачен', 'preparing' => 'в приготовлении', 'ready' => 'готов', 'done' => 'выдан'];
        $providerMap = ['tinkoff_sbp' => 'СБП', 'internal_balance' => 'звёздочки', 'internal_balance_rub' => 'рублёвый баланс'];
        foreach ($ps->fetchAll() as $row) {
            $payload = json_decode((string)($row['payload_json'] ?? ''), true) ?: [];
            $total = (float)($payload['amount_total'] ?? (float)$row['amount']);
            $cashbackSpend = (float)($payload['cashback_spend'] ?? 0);
            $meta = ($payStatusMap[$row['status']] ?? (string)$row['status']) . ' · ' . (string)$row['external_order_id'] . ' · ' . ($providerMap[(string)$row['provider']] ?? (string)$row['provider']);
            if ($cashbackSpend > 0) {
